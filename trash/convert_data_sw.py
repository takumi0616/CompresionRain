...
def process_and_save(args, mode):
    ...
    try:
        ...
        temp_creation_end = time.time()
        temp_creation_time = temp_creation_end - temp_creation_start
        
        print(f"INFO: Temporary file creation completed in {temp_creation_time:.2f} seconds")
        ...
        print(f"INFO: Post-processing completed in {post_processing_time:.2f} seconds")
        
        # 合計時間とその他の詳細情報
        total_measured_time = temp_creation_time + post_processing_time
        print(f"\n--- Performance Summary ---")
        print(f"Temporary file creation: {temp_creation_time:.2f} seconds ({temp_creation_time/total_measured_time*100:.1f}%)")
        print(f"Post-processing: {post_processing_time:.2f} seconds ({post_processing_time/total_measured_time*100:.1f}%)")
        print(f"Total measured time: {total_measured_time:.2f} seconds")
