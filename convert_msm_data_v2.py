...
def calc_stats():
    ...
    try:
        # --- 一時ファイル作成 ---
        logging.info("Starting parallel processing to create temporary files for stats...")
        ...
        with xr.open_mfdataset(sorted(temp_files), engine='h5netcdf', combine='by_coords', parallel=True) as ds:
            logging.info(f"  - open_mfdataset completed. Time taken: {time.time() - t_start_calc:.2f} seconds.")
            
            t_start_compute = time.time()
            means = ds.mean('time').compute()
            stds = ds.std('time').compute()
            logging.info(f"  - mean/std computation completed. Time taken: {time.time() - t_start_compute:.2f} seconds.")
            ...
            logging.info(f"  - Saving statistics to file completed. Time taken: {time.time() - t_start_save:.2f} seconds.")
            logging.info(f"Statistics successfully saved to: {STATS_FILE}")

    finally:
        # 一時ファイルを削除しないようにコメントアウト
        # if temp_dir.exists():
        #     shutil.rmtree(temp_dir)
        #     logging.info(f"Removed temporary directory: {temp_dir}")
        logging.info(f"Kept temporary directory for stats: {temp_dir}")
    logging.info(f"Finished [calc_stats] mode in {time.time() - total_start_time:.2f} seconds.")
...
def convert_monthly_data(year, month, stats_ds):
    ...
    try:
        # --- 一時ファイル作成 ---
        logging.info("Starting parallel processing to create temporary files...")
        t_start_parallel = time.time()
        ...
        with xr.open_mfdataset(sorted(temp_files), engine='h5netcdf', combine='by_coords', parallel=True, chunks={'time': 10}) as ds:
            logging.info(f"  - open_mfdataset completed. Time taken: {time.time() - t_start_concat:.2f} seconds.")
            ...
            logging.info(f"  - Standardization loop completed. Time taken: {time.time() - t_start_std:.2f} seconds.")
